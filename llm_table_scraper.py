...
- investment_type: Loan type (First Lien Senior Secured, Second Lien, etc.) or equity type (Common Equity, Preferred Equity)
- industry: Business sector (Healthcare, Technology, Manufacturing, etc.)
...
- reference_rate: Base rate (SOFR, LIBOR, Prime, etc.)
...
2. investment_type: Loan/Equity type. KEEP IT CONCISE (e.g., "First Lien Senior Secured" instead of "First lien senior secured loan, 8.5% due 2028"). Focus on the instrument type.
3. industry: Business sector (Healthcare, Software, etc.).
...
6. reference_rate: Base rate (SOFR, LIBOR, SF, Prime, P, n/a, etc.)
...
"""
    ...
    def process_historical_filings(self, ticker: str, filing_type: str = "10-Q",
        ...
        results = []
        for f in filings:
            logger.info(f"Processing filing from {f['date']} ({f['accession']})")
            
            # We use process_filing but we need to make sure it hits the right URL.
            # The current process_filing calls get_filing_index_url with year=year, 
            # which might return the wrong filing if multiple exist in a year.
            # However, for now, we'll follow the pattern in process_mrcc_historical.py 
            # and temporarily override get_filing_index_url if needed, or better, 
            # refactor process_filing to accept an index_url.
            
            # Let's refactor process_filing to accept an optional index_url.
